...
def deposit(initial_sum, percent, fixed_period, set_period):
    ...
    per = percent / 100 + 1    
    growth = per ** (set_period / fixed_period)
    return f"One month yield: {initial_sum * per ** (1 / 12)}\n" \
           f"One year yield: {initial_sum * per}\n" \
           f"Five year yield: {initial_sum * per ** 5}\n" \
           f"Ten year yield: {initial_sum * per ** 10}\n" \
           f"Set period yield: {initial_sum * growth}"
